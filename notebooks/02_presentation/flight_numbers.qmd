...
import tomli  # For reading TOML files. Install with: pip install tomli

class ProjectSetup:
    """
    Handles project path setup and configuration file loading
    """
    def __init__(self):
        self.project_root = self.setup_project_path()
        self.config = self.load_config()
    
    def setup_project_path(self):
        """
        Set up the Python path to include the project root directory.
        Returns the project root Path object.
        """
        current_file = Path().absolute()
        
        # Search upwards for marker files
        marker_files = ['.git', 'pyproject.toml', 'setup.py', 'requirements.txt', 'config.toml']
        
        root_dir = current_file
        while root_dir.parent != root_dir:  # Stop at filesystem root
            if any((root_dir / marker).exists() for marker in marker_files):
                break
        ...
        if root_str not in sys.path:
            ...
            print(f"Added project root to Python path: {root_str}")
        
        return root_dir
    
    def load_config(self):
        """
        Load the config.toml file from various possible locations.
        Returns the config dictionary.
        """
        possible_config_locations = [
            self.project_root / 'config.toml',
            self.project_root / 'config' / 'config.toml',
            self.project_root / 'conf' / 'config.toml',
            self.project_root / 'settings' / 'config.toml'
        ]
        
        # Print all searched locations for debugging
        print("\nSearching for config.toml in:")
        for loc in possible_config_locations:
            print(f"  {loc}")
            if loc.exists():
                print(f"\nFound config.toml at: {loc}")
                try:
        ...
        searched_paths = "\n  ".join(str(p) for p in possible_config_locations)
        raise FileNotFoundError(
            f"Could not find config.toml in any of these locations:\n  {searched_paths}\n"
...
The physics of these discs is very interesting and surprisingly complex; very small differences in the physical attributes, including shape, ratios, and plastic blends of the disc can result in a surprising differences in how two discs fly.
...
![](images/pdga_disc_specifications.png){fig-alt="Diagram of disc measurements. Source: PDGA Technical Specifications"}
...
In the below correlation matrix, we can see some clear relationships. We'll explore these further in the following section
...
```{python}
sns.scatterplot(df, x='speed', y='rim_thickness_(cm)')
```
...
~
Turn describes how a disc behaves during the high spin phase. Considering a typical right handed player throwing a backhand shot on a neutral angle, Turn indicates how much the disc will move tangent to its rotation (clockwise in this example), or simply put, how far right it will drift during its flight.
...
As expected, we see an opposite relationship between Turn and Fade, in both directionality and strength, as seen in @fig-corr-plot.
